...
public class Log {
	...
	static {
		...
		File screenShotFolder = new File(Reporter.getCurrentTestResult()
				.getTestContext().getOutputDirectory());
		screenShotFolderPath = screenShotFolder.getParent() + File.separator
		...
		final Map<String, String> params = Reporter.getCurrentTestResult()
				.getTestContext().getCurrentXmlTest().getAllParameters();

		if (params.containsKey("printconsoleoutput")) {
			Log.printconsoleoutput = Boolean.parseBoolean(params
					.get("printconsoleoutput"));
		}
	...
	public static String takeScreenShot(WebDriver driver) {
		...
		ScreenshotManager.takeScreenshot(driver, screenShotFolderPath
				+ inputFile);
		return inputFile;
	}
	...
	public static String getScreenShotHyperLink(String inputFile) {
		...
		screenShotLink = "<a href=\"." + File.separator + "ScreenShot"
	...
	public static void addTestRunMachineInfo(WebDriver driver) {
		...
		try {
			hub = (Reporter.getCurrentTestResult().getHost() == null) ? Inet4Address
					.getLocalHost().getHostName() : Reporter
					.getCurrentTestResult().getHost();
		...
		try {
			testMachine = "(Browser: "
					...
					+ hub
					+ ", Node: "
					+ WebDriverFactory.getTestSessionNodeIP(driver)
							.toUpperCase() + ")";
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void testCaseInfo(String description) {
		...
		if (Reporter.getOutput(Reporter.getCurrentTestResult()).toString()
			...
			Reporter.log(TEST_TITLE_HTML_BEGIN + description
	...
	public static void testCaseResult() {
		String reporterOutput = Reporter.getOutput(
				Reporter.getCurrentTestResult()).toString();
		...
		/*else if (reporterOutput.contains("not found in page")){
			fail("Test Failed. Check the steps above in red color.");
		} else if (reporterOutput.contains("timeout")){
			fail("Test Failed. Check the steps above in red color."); 
		} else if (reporterOutput.contains("unknown error")){
			fail("Test Failed. Check the steps above in red color."); 
		} */
		else {
			pass("Test Passed.");
	...
	public static void heading(String description, WebDriver... driver) {

		if (driver.length == 1) {
			String inputFile = takeScreenShot(driver[0]);
			Reporter.log(MESSAGE_HTML_BEGIN + "<b>" + description + "&emsp;"
					+ getScreenShotHyperLink(inputFile) + "</b>"
					+ MESSAGE_HTML_END);
			ExtentReporter.info(description + " "
					+ getScreenShotHyperLink(inputFile));
		} else {
			Reporter.log(MESSAGE_HTML_BEGIN + "<b>" + description + "</b>"
					+ MESSAGE_HTML_END);
			lsLog4j().log(callerClass(), Level.INFO, description, null);
			ExtentReporter.info(description);
		}
	...
	public static void message(String description, WebDriver driver,
		...
		if (takeScreenShot.length > 0
				&& takeScreenShot[0].equals(Boolean.FALSE)) {
			finalDecision = false;
		}
		if (configProperty.getProperty("isTakeScreenShot") != null
				&& configProperty.getProperty("isTakeScreenShot")
		...
		if (finalDecision) {
			...
			Reporter.log(MESSAGE_HTML_BEGIN + description + "&emsp;"
	 ...
	 * @throws IOException
	 */
	public static void message(WebDriver driver, String description,
			String screenshotfolderpath, String screenshotfileName)
			throws IOException {
		...
		FileUtils.copyFile(screenshot, new File(screenshotfolderpath
				+ File.separator + inputFile));
		FileUtils.copyFile(screenshot, new File(screenShotFolderPath
				+ File.separator + inputFile));
		Reporter.log(MESSAGE_HTML_BEGIN
				+ String.format("%-75s", description).replace(" ", "&nbsp;")
				+ "---&emsp;" + getScreenShotHyperLink(inputFile)
				+ MESSAGE_HTML_END);
		ExtentReporter.info(String.format("%-75s", description).replace(" ",
				"&nbsp;")
				+ "---&emsp;" + getScreenShotHyperLink(inputFile));
		lsLog4j().log(callerClass(), Level.INFO, description, null);
	...
	public static void message(String description, String screenshot)
		...
		try {
			String inputFile = Reporter.getCurrentTestResult().getName() + "_"
					+ screenShotIndex.incrementAndGet() + ".png";
			FileUtils.copyFile(new File(screenshot), new File(
					screenShotFolderPath + inputFile));
	...
	public static void event(String description) {
		...
		String currDate = new SimpleDateFormat("dd MMM HH:mm:ss SSS")
				.format(Calendar.getInstance().getTime());
		Reporter.log(EVENT_HTML_BEGIN + currDate + " - " + description
				+ EVENT_HTML_END);
	...
	public static void event(String description, long duration) {
		...
		String currDate = new SimpleDateFormat("dd MMM HH:mm:ss SSS")
				.format(Calendar.getInstance().getTime());
		Reporter.log(EVENT_HTML_BEGIN + currDate + " - <b>" + duration
				+ "</b> - " + description + " - "
				+ Thread.currentThread().getStackTrace()[2].toString()
				+ EVENT_HTML_END);
		lsLog4j().log(callerClass(), Level.DEBUG, description, null);
		ExtentReporter.debug(currDate + " - <b>" + duration + "</b> - "
				+ description + " - "
				+ Thread.currentThread().getStackTrace()[2].toString());
	}
	...
	public static void pass(String description, WebDriver driver) {
		if (configProperty.getProperty("isTakeScreenShot") != null
			...
			String inputFile = takeScreenShot(driver);
			Reporter.log(PASS_HTML_BEGIN + "Actual : " + description
					+ PASS_HTML_END1 + getScreenShotHyperLink(inputFile)
					+ PASS_HTML_END2);
			ExtentReporter.pass(PASS_HTML_BEGIN + "Actual : " + description + PASS_HTML_END1 
	...
	public static void fail(String description, WebDriver driver) {
		String inputFile = takeScreenShot(driver);
		Reporter.log("<!--FAIL-->");
		Reporter.log(FAIL_HTML_BEGIN + description + FAIL_HTML_END1
	...
	public static void fail(String description) {
		lsLog4j().log(callerClass(), Level.ERROR, description, null);
		Reporter.log("<!--FAIL-->");
		Reporter.log(FAIL_HTML_BEGIN + "Actual : "+description + FAIL_HTML_END1
				+ FAIL_HTML_END2);
		ExtentReporter.fail(description);
		ExtentReporter.logStackTrace(new AssertionError("Actual : "+description));
		Assert.fail("Actual : "+description);
	}
	...
	public static boolean hasFailSofts() {
		return Reporter.getOutput(Reporter.getCurrentTestResult()).toString()
				.contains("FAILSOFT");
	}
	...
	public static void failsoft(String description, WebDriver driver) {
		...
		String inputFile = takeScreenShot(driver);
		Reporter.log("<b><div class=\"test-result\"><font color=\"red\">&emsp; Actual : "
				+ description
				+ "</font>"
				+ getScreenShotHyperLink(inputFile)
				+ "</div></b>");
		Reporter.log("<!--FAILSOFT-->");
		ExtentReporter.fail("Actual : " + description + " "
				+ getScreenShotHyperLink(inputFile));
	...
	public static void failsoft(String description) {
		Reporter.log("<!--FAILSOFT-->");
		Reporter.log("<div class=\"test-result\"><font color=\"red\">&emsp;Actual : "
	 ...
	 *            to take screenshot
	 */
	public static String buildfailsoftMessage(String description,
			WebDriver driver) {

		String inputFile = takeScreenShot(driver);
		return "<div class=\"test-result\">&emsp; <font color=\"red\"><strong>"
	 ...
	 */
	public static void assertThat(boolean status, String passmsg,
			String failmsg, WebDriver driver) {
		if (!status) {
	 ...
	 *            -message to be logged when assert status is false
	 */
	public static void softAssertThat(boolean status, String passmsg,
			String failmsg) {
		if (!status) {
			Log.failsoft(failmsg);
		} else {
			Log.message( "Actual : "+ passmsg);
		}
	}
	...
	public static void addSauceJobUrlToReport(WebDriver driver, String sauceLink) {
		...
		params[0] = (params[0] == null || !params[0].toString().contains(
				"SauceLab link")) ? "SauceLab link: <a href='" + sauceLink
				+ "' target='_blank'>" + sauceLink + "</a>" : params[0]
				+ "<br>SauceLab link: <a href='" + sauceLink
				+ "' target='_blank'>" + sauceLink + "</a>";
		Reporter.getCurrentTestResult().setParameters(params);
		ExtentReporter.addAttribute("SauceLab link: <a href='" + sauceLink
				+ "' target='_blank'>" + sauceLink + "</a>");
	}
}